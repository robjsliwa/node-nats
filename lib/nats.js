...
Client.prototype.parseOptions = function(opts) {
    ...
    if(options.useOldRequestStyle) {
        Client.prototype.requestOne = this.oldRequestOne;
    }
};
...
Client.prototype.unsubscribe = function(sid, opt_max) {
    ...
    // in the case of new requestOne, it is possible they want perform
    // an unsubscribe with the returned 'sid'. Intercept that and clear
    // the request configuration.
    if(sid.token && sid.timeout) {
        // this is cancel on a global request/reply
        delete this.client.respMap[sid.token];
        clearTimeout(sid.timeout);
        return;
    }
...
Client.prototype.requestOne = function(subject, opt_msg, opt_options, timeout, callback) {
    ...
    conf.timeout = setTimeout(function() {
        ...
        delete client.resp.respMap[token];
    }, timeout);
 ...
 * @api private
 */
Client.prototype.respToken = function(subject) {
    return subject.substr(this.resp.inboxPrefixLen);
};

/**
 * Response handler for new requestOne.
 *
 * @api private
 */
Client.prototype.respHandler = function(msg, reply, subject) {
    var client = this;
    var respToken = client.respToken(subject);
    var conf = client.resp.respMap[respToken];
    delete client.resp.respMap[respToken];
    if(conf) {
        if(conf.timeout) {
...
Client.prototype.createResponseMux = function() {
    if(!this.resp) {
        ...
        var sid = this.subscribe(ginbox, function(msg, reply, subject) {
            client.respHandler(msg, reply, subject);
        });
        this.resp = {};
        this.resp.inbox = inbox;
        this.resp.inboxPrefixLen = inbox.length + 1;
        this.resp.subscriptionID = sid;
        this.resp.respMap = {};
    }
    return this.resp.inbox;
};
