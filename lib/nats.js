...
Client.prototype.parseOptions = function(opts) {
    ...
    // Set jwt as needed if in options.
    client.jwt = options.jwt

    // Authentication - make sure authentication is valid.
    if (client.user && (client.token || client.jwt)) {
...
Client.prototype.selectServer = function() {
    ...
    if ('auth' in server.url && !!server.url.auth) {
        ...
        if (auth.length !== 1) {
            ...
            /*if (client.options.jwt === undefined) {
                client.jwt = auth[0];
            } else*/ if (client.options.token === undefined) {
                client.token = auth[0];
            }
        }
    }
